 ...
 */
template<typename T, std::size_t N>
class Circular_Buffer<T, N, With_Interrupt_Support, With_Overflow_Underflow_Protection> {
  public:
    static_assert( N > 0 );
     ...
     *         buffer is full.
     */
    auto push_from_interrupt( Value && value ) noexcept -> Result<Void, Error_Code>
    {
        if ( full_from_interrupt() ) {
            return Generic_Error::CIRCULAR_BUFFER_WOULD_OVERFLOW;
        } // if
     ...
     *         buffer is full.
     */
    auto push_from_interrupt( Value const & value ) noexcept -> Result<Void, Error_Code>
    {
        if ( full_from_interrupt() ) {
            return Generic_Error::CIRCULAR_BUFFER_WOULD_OVERFLOW;
        } // if
     ...
     */
    template<typename... Arguments>
    auto emplace_from_interrupt( Arguments &&... arguments ) noexcept -> Result<Void, Error_Code>
    {
        if ( full_from_interrupt() ) {
            return Generic_Error::CIRCULAR_BUFFER_WOULD_OVERFLOW;
        } // if
     ...
     *         buffer is empty.
     */
    auto pop_from_interrupt() noexcept -> Result<Void, Error_Code>
    {
        if ( empty_from_interrupt() ) {
            return Generic_Error::CIRCULAR_BUFFER_WOULD_UNDERFLOW;
        } // if
