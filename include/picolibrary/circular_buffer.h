...
/**
 * \brief Single reader, single writer circular buffer.
 *
 * \tparam T The circular buffer element type.
 * \tparam Size_Type The unsigned integer type used to track the number of elements in the
 *         circular buffer.
 ...
 *         two).
 * \tparam Interrupt_Controller The type of interrupt controller used to manipulate the
 *         interrupt enable state of the interrupts that interact with the circular
 *         buffer.
 */
template<typename T, typename Size_Type, Size_Type N, typename Interrupt_Controller>
class Circular_Buffer<T, Size_Type, N, Single_Reader_Single_Writer, Interrupt_Controller> {
  public:
    static_assert( std::is_unsigned_v<Size_Type> );
     ...
     * \brief Constructor.
     *
     * \param[in] interrupt_controller The interrupt controller used to manipulate the
     *            interrupt enable state of the interrupts that interact with the circular
     *            buffer.
     */
    ...
    auto size() const noexcept -> Size
    {
        auto guard = Interrupt::Critical_Section_Guard{ m_interrupt_controller,
    ...
    void push_from_interrupt( Value && value ) noexcept
    {
        ...
        push_from_interrupt( BYPASS_PRECONDITION_EXPECTATION_CHECKS, std::move( value ) );
    }
    ...
    void push_from_interrupt( Value const & value ) noexcept
    {
        ...
        push_from_interrupt( BYPASS_PRECONDITION_EXPECTATION_CHECKS, value );
    }
  ...
  private:
    ...
    /**
     * \brief interrupt_controller The interrupt controller used to manipulate the
     *        interrupt enable state of the interrupts that interact with the circular
     *        buffer.
     */
...
/**
 * \brief Single reader, multiple writers circular buffer.
 *
 * \tparam T The circular buffer element type.
 * \tparam Size_Type The unsigned integer type used to track the number of elements in the
 *         circular buffer.
 ...
 */
template<typename T, typename Size_Type, Size_Type N, typename Interrupt_Controller>
class Circular_Buffer<T, Size_Type, N, Single_Reader_Multiple_Writers, Interrupt_Controller> {
  public:
    static_assert( std::is_unsigned_v<Size_Type> );
