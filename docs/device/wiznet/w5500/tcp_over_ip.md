...
- To get a socket's hardware socket IDs, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::socket_ids()` member function.
- To get a socket's socket interrupt mask (mask used when checking the network stack's
  socket interrupt context), use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::socket_interrupt_mask()` member
  function.
- To configure a socket's no delayed ACK usage, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::configure_no_delayed_ack_usage()` member
  function.
- To get a socket's no delayed ACK usage configuration, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::no_delayed_ack_usage_configuration()`
  member function.
- To configure a socket's maximum segment size, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::configure_maximum_segment_size()` member
  function.
- To get a socket's maximum segment size, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::maximum_segment_size()` member function.
- To configure a socket's IPv4 packet time to live field value, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::configure_time_to_live()` member
  function.
- To get a socket's IPv4 packet time to live field value, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::time_to_live()` member function.
- To configure a socket's keepalive packet transmission period (SN_KPALVTR register
  value), use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::configure_keepalive_period()` member
  function.
- To get a socket's keepalive packet transmission period (SN_KPALVTR register value), use
  the `::picolibrary::WIZnet::W5500::IP::TCP::Server::keepalive_period()` member function.
- To enable interrupts, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::enable_interrupts()` member function.
- To disable interrupts, use the
  `::picolibrary::WIZnet::W5500::IP::TCP::Server::disable_interrupts()` member functions.
- To get a mask identifying the interrupts that are enabled, use the
