...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_dport() properly handles a
 *        read error.
 */
TEST( readSNDPORT, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_dport( random<Socket_ID>() );
...
TEST( readSNDPORT, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x0010, _ ) )
        .WillOnce( Return( convert_data_to_vector( data ) ) );

    auto const result = w5500.read_sn_dport( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_dport() properly handles a
 *        write error.
 */
TEST( writeSNDPORT, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_dport( random<Socket_ID>(), random<std::uint16_t>() );
...
TEST( writeSNDPORT, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x0010, convert_data_to_vector( data ) ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_dport( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_mssr() properly handles a
 *        read error.
 */
TEST( readSNMSSR, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_mssr( random<Socket_ID>() );
...
TEST( readSNMSSR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x0012, _ ) )
        .WillOnce( Return( convert_data_to_vector( data ) ) );

    auto const result = w5500.read_sn_mssr( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_mssr() properly handles a
 *        write error.
 */
TEST( writeSNMSSR, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_mssr( random<Socket_ID>(), random<std::uint16_t>() );
...
TEST( writeSNMSSR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x0012, convert_data_to_vector( data ) ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_mssr( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_tos() properly handles a read
 *        error.
 */
TEST( readSNTOS, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_tos( random<Socket_ID>() );
...
TEST( readSNTOS, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x0015 ) ).WillOnce( Return( data ) );

    auto const result = w5500.read_sn_tos( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_tos() properly handles a
 *        write error.
 */
TEST( writeSNTOS, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_tos( random<Socket_ID>(), random<std::uint8_t>() );
...
TEST( writeSNTOS, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x0015, data ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_tos( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_ttl() properly handles a read
 *        error.
 */
TEST( readSNTTL, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_ttl( random<Socket_ID>() );
...
TEST( readSNTTL, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x0016 ) ).WillOnce( Return( data ) );

    auto const result = w5500.read_sn_ttl( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_ttl() properly handles a
 *        write error.
 */
TEST( writeSNTTL, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_ttl( random<Socket_ID>(), random<std::uint8_t>() );
...
TEST( writeSNTTL, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x0016, data ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_ttl( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_rxbuf_size() properly handles
 *        a read error.
 */
TEST( readSNRXBUFSIZE, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_rxbuf_size( random<Socket_ID>() );
...
TEST( readSNRXBUFSIZE, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x001E ) ).WillOnce( Return( data ) );

    auto const result = w5500.read_sn_rxbuf_size( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_rxbuf_size() properly
 *        handles a write error.
 */
TEST( writeSNRXBUFSIZE, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_rxbuf_size( random<Socket_ID>(), random<std::uint8_t>() );
...
TEST( writeSNRXBUFSIZE, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x001E, data ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_rxbuf_size( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_txbuf_size() properly handles
 *        a read error.
 */
TEST( readSNTXBUFSIZE, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_txbuf_size( random<Socket_ID>() );
...
TEST( readSNTXBUFSIZE, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x001F ) ).WillOnce( Return( data ) );

    auto const result = w5500.read_sn_txbuf_size( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_txbuf_size() properly
 *        handles a write error.
 */
TEST( writeSNTXBUFSIZE, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_txbuf_size( random<Socket_ID>(), random<std::uint8_t>() );
...
TEST( writeSNTXBUFSIZE, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x001F, data ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_txbuf_size( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_tx_fsr() properly handles a
 *        read error.
 */
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_tx_rd() properly handles a
 *        read error.
 */
TEST( readSNTXRD, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_tx_rd( random<Socket_ID>() );
...
TEST( readSNTXRD, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x0022, _ ) )
        .WillOnce( Return( convert_data_to_vector( data ) ) );

    auto const result = w5500.read_sn_tx_rd( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_tx_wr() properly handles a
 *        read error.
 */
TEST( readSNTXWR, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_tx_wr( random<Socket_ID>() );
...
TEST( readSNTXWR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x0024, _ ) )
        .WillOnce( Return( convert_data_to_vector( data ) ) );

    auto const result = w5500.read_sn_tx_wr( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_tx_wr() properly handles a
 *        write error.
 */
TEST( writeSNTXWR, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_tx_wr( random<Socket_ID>(), random<std::uint16_t>() );
...
TEST( writeSNTXWR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x0024, convert_data_to_vector( data ) ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_tx_wr( socket_id, data ).is_error() );
}
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_rx_rd() properly handles a
 *        read error.
 */
TEST( readSNRXRD, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_rx_rd( random<Socket_ID>() );
...
TEST( readSNRXRD, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x0028, _ ) )
        .WillOnce( Return( convert_data_to_vector( data ) ) );

    auto const result = w5500.read_sn_rx_rd( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_rx_rd() properly handles a
 *        write error.
 */
TEST( writeSNRXRD, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_rx_rd( random<Socket_ID>(), random<std::uint16_t>() );
...
TEST( writeSNRXRD, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x0028, convert_data_to_vector( data ) ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_rx_rd( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_rx_wr() properly handles a
 *        read error.
 */
TEST( readSNRXWR, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_rx_wr( random<Socket_ID>() );
...
TEST( readSNRXWR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x002A, _ ) )
        .WillOnce( Return( convert_data_to_vector( data ) ) );

    auto const result = w5500.read_sn_rx_wr( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_imr() properly handles a read
 *        error.
 */
TEST( readSNIMR, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_imr( random<Socket_ID>() );
...
TEST( readSNIMR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x002C ) ).WillOnce( Return( data ) );

    auto const result = w5500.read_sn_imr( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_imr() properly handles a
 *        write error.
 */
TEST( writeSNIMR, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_imr( random<Socket_ID>(), random<std::uint8_t>() );
...
TEST( writeSNIMR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x002C, data ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_imr( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_frag() properly handles a
 *        read error.
 */
TEST( readSNFRAG, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_frag( random<Socket_ID>() );
...
TEST( readSNFRAG, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x002D, _ ) )
        .WillOnce( Return( convert_data_to_vector( data ) ) );

    auto const result = w5500.read_sn_frag( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_frag() properly handles a
 *        write error.
 */
TEST( writeSNFRAG, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_frag( random<Socket_ID>(), random<std::uint16_t>() );
...
TEST( writeSNFRAG, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x002D, convert_data_to_vector( data ) ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_frag( socket_id, data ).is_error() );
}

/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::read_sn_kpalvtr() properly handles a
 *        read error.
 */
TEST( readSNKPALVTR, readError )
{
    auto const w5500 = Driver{};
    ...
    auto const result = w5500.read_sn_kpalvtr( random<Socket_ID>() );
...
TEST( readSNKPALVTR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, read( socket_id, Region::REGISTERS, 0x002F ) ).WillOnce( Return( data ) );

    auto const result = w5500.read_sn_kpalvtr( socket_id );
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Driver::write_sn_kpalvtr() properly handles a
 *        write error.
 */
TEST( writeSNKPALVTR, writeError )
{
    auto w5500 = Driver{};
    ...
    auto const result = w5500.write_sn_kpalvtr( random<Socket_ID>(), random<std::uint8_t>() );
...
TEST( writeSNKPALVTR, worksProperly )
{
    ...
    EXPECT_CALL( w5500, write( socket_id, Region::REGISTERS, 0x002F, data ) )
        .WillOnce( Return( Result<Void, Error_Code>{} ) );

    EXPECT_FALSE( w5500.write_sn_kpalvtr( socket_id, data ).is_error() );
}
