...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Network_Stack::TCP_Socket::shutdown()
 *        properly handles an SN_SR register read error.
 */
 ...
 *        properly handles a nonresponsive device error when reading the SN_SR register.
 */
TEST( shutdown, nonresponsiveDeviceErrorSNSR )
{
    struct {
        std::uint8_t sn_sr;
    } const test_cases[]{
 ...
 *        properly handles a nonresponsive device error when reading the SN_CR register.
 */
TEST( shutdown, nonresponsiveDeviceErrorSNCR )
{
    struct {
        std::uint8_t sn_cr;
    } const test_cases[]{
        { random<std::uint8_t>( 0x01, 0x07 ) },
        { random<std::uint8_t>( 0x09, 0xFF ) },
    };
...
/**
 * \brief Verify picolibrary::WIZnet::W5500::Network_Stack::TCP_Socket::shutdown() works
 *        properly.
 */
TEST( shutdown, worksProperly )
{
    ...
    EXPECT_CALL( driver, read_sn_cr( socket_id ) ).WillOnce( Return( static_cast<std::uint8_t>( 0x08 ) ) );
    EXPECT_CALL( driver, read_sn_cr( socket_id ) ).WillOnce( Return( static_cast<std::uint8_t>( 0x00 ) ) );

    EXPECT_FALSE( socket.shutdown().is_error() );
}
